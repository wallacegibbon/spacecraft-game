...
Game::Game(int width, int height, QWidget *parent) {
	...
	explosion_sound_1_ = new CuteSoundPlayer(
		"qrc:/sound/explosion_1.wav", this
	);
	explosion_sound_2_ = new CuteSoundPlayer(
		"qrc:/sound/explosion_2.wav", this
	);
	...
	*/
	...
	connect(
		this,
		&Game::enemy_destroyed,
		this,
		&Game::play_enemy_explosion
	);
	...
	connect(
		this,
		&Game::refocus_keyboard,
		this,
		&Game::handle_refocus_keyboard
	);
...
void Game::display_main_menu() {
	...
	QGraphicsTextItem *title = new QGraphicsTextItem(
		QString("Spacecraft Game"),
		common_dialog_
	);

	title->setFont(QFont("sans", 18));
	title->setPos(
		(common_dialog_->rect().width() -
	...
	play_button->setPos(
		(common_dialog_->rect().width() -
			play_button->boundingRect().width()) / 2,
		130
	);
	...
	quit_button->setPos(
		(common_dialog_->rect().width() -
			quit_button->boundingRect().width()) / 2,
		190
	);
...
void Game::display_replay_menu() {
	...
	replay_button->setPos(
		(common_dialog_->rect().width() -
			replay_button->boundingRect().width()) / 2,
		130
	);
...
void Game::update_score() {
	score_->update_score_display();
}

void Game::play_enemy_explosion() const {
	explosion_sound_1_->play();
}
