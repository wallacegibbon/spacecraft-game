...
Game::Game(int width, int height, QWidget *parent) {
	...
	explosion_sound_1 =
		new CuteSoundPlayer("qrc:/sound/explosion_1.wav", this);
	explosion_sound_2 =
		new CuteSoundPlayer("qrc:/sound/explosion_2.wav", this);

	bgm_sound =
		new CircleAudioPlayer("qrc:/sound/bgm_0.ogg", this);
	...
	*/
	...
	connect(this, &Game::enemy_destroyed,
		this, &Game::play_enemy_explosion);

	connect(this, &Game::start, this, &Game::start_game);
	connect(this, &Game::stop, this, &Game::stop_game);

	connect(this, &Game::refocus_keyboard,
		this, &Game::handle_refocus_keyboard);

	refresh_timer = new QTimer(this);
	enemy_creating_timer = new QTimer(this);

	connect(refresh_timer, &QTimer::timeout,
		this, &Game::static_item_handler);

	/*
	QObject::connect(enemy_timer, &QTimer::timeout,
		this, &Game::spawn_enemy);
	*/

	connect(enemy_creating_timer, &QTimer::timeout,
		this, &Game::spawn_enemy);
...
void Game::display_main_menu() {
	...
	QGraphicsTextItem *title =
		new QGraphicsTextItem(QString("Airplane Game"), common_dialog);

	title->setFont(QFont("sans", 18));
	title->setPos((common_dialog->rect().width()
			- title->boundingRect().width()) / 2, 50);

	Button *play_button = new Button("Play", common_dialog);
	play_button->setPos((common_dialog->rect().width() -
			play_button->boundingRect().width()) / 2, 130);
	...
	quit_button->setPos((common_dialog->rect().width() -
			quit_button->boundingRect().width()) / 2, 190);
...
void Game::display_replay_menu() {
	prepare_common_dialog();
	QString end_text = QString("Game Over, your score: %1")
		.arg(player->get_score());

	QGraphicsTextItem *title =
		new QGraphicsTextItem(end_text, common_dialog);
	...
	replay_button->setPos((common_dialog->rect().width() -
			replay_button->boundingRect().width()) / 2, 130);
...
void Game::start_game() {
	...
	connect(keyboard_controller, &KeyboardController::command,
		this, &Game::keyboard_handler);

	keyboard_controller->setFocus();

	player = new Airplane();
	player->setPos(view->width() / 2 - player->width() / 2,
		view->height() - player->height() - 10);
