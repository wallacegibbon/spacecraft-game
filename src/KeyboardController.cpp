...
KeyboardController::KeyboardController(
  ...
  connect(
    repeat_timer_, &QTimer::timeout, this, &KeyboardController::repeat_handler
  );
...
void KeyboardController::repeat_handler() {
  /*
  qDebug() << "repeat handler is triggered, press_stack_: "
          << press_stack_;
  */
...
void KeyboardController::keyPressEvent(QKeyEvent *event) {
  ...
  qDebug() << "pressed: " << event->key() << ", press_stack_: " << press_stack_;
...
void KeyboardController::keyReleaseEvent(QKeyEvent *event) {
  ...
  if (press_stack_ > 0 && is_command_key(event->key())) { press_stack_--; }
  if (press_stack_ == 0) { repeat_timer_->stop(); }
